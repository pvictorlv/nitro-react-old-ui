...
import {
    ...
    Flex, NitroCardContentView, NitroCardHeaderView,
    NitroCardView,
    Text
} from '../../../../common';
...
export const AvatarInfoRentableBotChatView: FC<AvatarInfoRentableBotChatViewProps> = props =>
{
    ...
    return (
        <NitroCardView uniqueKey={ 'bot-change-speaker' }>
            <NitroCardHeaderView headerText={ LocalizeText('bot.skill.chatter.configuration.title') }
                                 onCloseClick={ onClose }/>

            <NitroCardContentView overflow={ 'hidden' }>
                <Column className="p-1">
                    <Column gap={ 1 }>
                        <Text variant="black">{ LocalizeText('bot.skill.chatter.configuration.chat.text') }</Text>
                        <textarea className="form-control form-control-sm"
                                  placeholder={ LocalizeText('bot.skill.chatter.configuration.text.placeholder') }
                                  value={ newText } rows={ 7 } onChange={ e => setNewText(e.target.value) }/>
                    </Column>
                    <Column gap={ 1 }>
                        <Flex gap={ 1 } alignItems="center" justifyContent="between">
                            <Text fullWidth
                                  variant="black">{ LocalizeText('bot.skill.chatter.configuration.automatic.chat') }</Text>
                            <input type="checkbox" className="form-check-input" checked={ automaticChat }
                                   onChange={ event => setAutomaticChat(event.target.checked) }/>
                        </Flex>
                        <Flex gap={ 1 } alignItems="center" justifyContent="between">
                            <Text fullWidth
                                  variant="black">{ LocalizeText('bot.skill.chatter.configuration.markov') }</Text>
                            <input type="checkbox" className="form-check-input" checked={ mixSentences }
                                   onChange={ event => setMixSentences(event.target.checked) }/>
                        </Flex>
                        <Flex gap={ 1 } alignItems="center" justifyContent="between">
                            <Text fullWidth
                                  variant="black">{ LocalizeText('bot.skill.chatter.configuration.chat.delay') }</Text>
                            <input type="number" className="form-control form-control-sm" value={ chatDelay }
