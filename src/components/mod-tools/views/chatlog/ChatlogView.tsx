                    ...
                    <InfiniteScroll rows={ allRecords } rowRender={ (row: ChatlogRecord) => 
                    {
                        return (
                            <>
                                ...
                                { !row.isRoomInfo &&
                                    <Grid fullHeight={ false } gap={ 1 } alignItems="center"
                                        className="log-entry py-1 border-bottom">
                                        <Text className="g-col-2">{ row.timestamp }</Text>
                                        <Text className="g-col-3" bold underline pointer
                                            onClick={ event => CreateLinkEvent(`mod-tools/open-user-info/${ row.habboId }`) }>{ row.username }</Text>
