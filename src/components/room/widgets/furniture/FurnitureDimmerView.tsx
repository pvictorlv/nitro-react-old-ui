...
export const FurnitureDimmerView: FC<{}> = props =>
{
    ...
    return (
        <NitroCardView className="nitro-room-widget-dimmer">
            ...
            { (dimmerState === 1) &&
                <NitroCardTabsView>
                    { presets.map(preset => <NitroCardTabsItemView key={ preset.id }
                                                                   isActive={ (selectedPresetId === preset.id) }
                                                                   onClick={ event => selectPresetId(preset.id) }>{ LocalizeText(`widget.dimmer.tab.${ preset.id }`) }</NitroCardTabsItemView>) }
                </NitroCardTabsView> }
            <NitroCardContentView>
                { (dimmerState === 0) &&
                ...
                { (dimmerState === 1) &&
                    <>
                        <Column gap={ 1 }>
                            ...
                            { !isFreeColorMode &&
                                <Grid gap={ 1 } columnCount={ 7 }>
                                    ...
                                    {
                                        return (
                                            <Column fullWidth pointer key={ index }
                        ...
                        <Flex alignItems="center" gap={ 1 }>
                            <input className="form-check-input" type="checkbox" checked={ (selectedEffectId === 2) }
                                   onChange={ event => setSelectedEffectId(event.target.checked ? 2 : 1) }/>
                            <Text className={ 'text-volter' }>{ LocalizeText('widget.dimmer.type.checkbox') }</Text>
                        </Flex>
                    </> }
            </NitroCardContentView>
            <Flex gap={ 1 } className={ 'justify-content-between p-1' }>
                { (dimmerState === 0) && <Button
                    onClick={ () => FurnitureDimmerUtilities.changeState() }>{ LocalizeText('widget.dimmer.button.on') }</Button> }
                { (dimmerState === 1) && <Button
                    onClick={ applyChanges }>{ LocalizeText('widget.dimmer.button.apply') }</Button> }
                { (dimmerState === 1) && <Button
                    onClick={ () => FurnitureDimmerUtilities.changeState() }>{ LocalizeText('widget.dimmer.button.off') }</Button> }
            </Flex>
        </NitroCardView>
    );
}
