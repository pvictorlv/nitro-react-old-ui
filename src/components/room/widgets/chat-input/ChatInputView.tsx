import {
    ...
    NitroPoint,
    RoomControllerLevel,
    RoomObjectCategory, RoomObjectType,
    RoomSessionChatEvent, SystemChatStyleEnum
} from '@nitrots/nitro-renderer';
import {FC, useCallback, useEffect, useMemo, useRef, useState} from 'react';
import {createPortal} from 'react-dom';
import {
    ChatBubbleMessage, ChatEntryType, ChatHistoryCurrentDate,
    ChatMessageTypeEnum,
    GetClubMemberLevel,
    GetConfiguration, GetRoomObjectScreenLocation,
    GetSessionDataManager,
    LocalizeText, RoomChatFormatter,
...
export const ChatInputView: FC<{}> = props => {
    ...
    const onPasteEvent = useCallback((event: ClipboardEvent) => {
        ...
        for (let index in items) {
            ...
            if (item.kind === 'file') {
                ...
                img.onload = function(){
                    ...
                    let text = '<a href="' + event + '">Imagem compartilhada, clique para abrir!</a>';
    ...
    useEffect(() => {
        ...
        return () => {
            document.body.removeEventListener('paste', onPasteEvent);
        }
    }, [onPasteEvent]);
                ...
                <div className="input-sizer align-items-center">
                    {!floodBlocked &&
                        <input ref={inputRef} type="text" className="chat-input"
                               placeholder={LocalizeText('widgets.chatinput.default')} value={chatValue}
                               maxLength={maxChatLength} onChange={event => updateChatInput(event.target.value)}
