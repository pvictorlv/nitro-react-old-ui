    ...
    const handleToolClick = (action: string, value?: string) => {
        switch (action) {
            case 'settings':
                ...
                return;
            case 'open-meet':
                CreateLinkEvent('meeting/toggle');
                return;
            case 'toggle-mute':
                CreateLinkEvent('meeting/toggle-mute');
                return;
            case 'zoom':
    ...
    return (
        <Flex className="nitro-room-tools-container" gap={2}>
            <Column center className="nitro-room-tools p-2">
                ...
                <Base pointer title={LocalizeText('room.zoom.button.text')} onClick={() => handleToolClick('zoom')}
                      className={classNames('icon', (!isZoomedIn && 'icon-zoom-less'), (isZoomedIn && 'icon-zoom-more'))}/>

                <Base pointer title={LocalizeText('room.meeting.toggle.button.text')} onClick={() => handleToolClick('open-meet')}
                      className={classNames('icon',   'icon-toggle-meet')}/>
                    ...
                    <Column center>
                        <Column className="nitro-room-tools-info rounded py-2 px-3">
                            ...
                            {roomTags && roomTags.length > 0 &&
                                <Flex gap={2}>
                                    {roomTags.map((tag, index) => <Text key={index} small pointer variant="white"
                                                                        className="rounded bg-primary p-1"
                                                                        onClick={() => handleToolClick('navigator_search_tag', tag)}>#{tag}</Text>)}
                                </Flex>}
                        </Column>
                    </Column>
                </TransitionAnimation>
