...
export const CatalogLayoutVipBuyView: FC<CatalogLayoutProps> = props =>
{
    ...
    const getPurchaseButton = useCallback(() =>
    {
        ...
        {
            return <Button
                fullWidth>{ LocalizeText('catalog.alert.notenough.activitypoints.title.' + pendingOffer.priceActivityPointsType) }</Button>;
        }

        switch (purchaseState)
        {
            case CatalogPurchaseState.CONFIRM:
                return <Button fullWidth variant="warning"
                               onClick={ purchaseSubscription }>{ LocalizeText('catalog.marketplace.confirm_title') }</Button>;
            ...
            default:
                return <Button fullWidth
                               onClick={ () => setPurchaseState(CatalogPurchaseState.CONFIRM) }>{ LocalizeText('buy') }</Button>;
        }
    ...
    return (
        <>
            ...
            <Grid className={'pt-4'}>
                ...
                <Column fullHeight size={ 7 } alignItems={ 'center' }  overflow="hidden" justifyContent="between">
                    <AutoGrid columnCount={ 1 } className="nitro-catalog-layout-vip-buy-grid">
                        ...
                        {
                            return (
                                <LayoutGridItem key={ index } column={ false } center={ false } alignItems="center"
                                                justifyContent="between" itemActive={ pendingOffer === offer }
                                                className="p-1" onClick={ () => setOffer(offer) }>
