...
{
    const {columnCount = 3, children = null, ...rest} = props;
    ...
    return (
        <AutoGrid innerRef={ elementRef } justifyContent={ 'center' } columnMinWidth={ 42 } columnCount={ columnCount }
                  className={ 'catalog-grid' } gap={ 1 } { ...rest }>
            { currentPage.offers && (currentPage.offers.length > 0) && currentPage.offers.map((offer, index) =>
                <CatalogGridOfferView key={ index }
                                      itemActive={ (currentOffer && (currentOffer.offerId === offer.offerId)) }
                                      offer={ offer } selectOffer={ selectOffer }/>) }
            { children }
        </AutoGrid>
    );
}
