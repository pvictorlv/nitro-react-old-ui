...
export const AvatarEditorWardrobeView: FC<AvatarEditorWardrobeViewProps> = props =>
{
    ...
    const figures = useMemo(() =>
    {
        ...
        savedFigures.forEach(([ figureContainer, gender ], index) =>
        {
            ...
            items.push(
                <LayoutGridItem key={ index } position="relative" overflow="hidden"
                                className="nitro-avatar-editor-wardrobe-figure-preview">
                    { figureContainer &&
                        <LayoutAvatarImageView figure={ figureContainer.getFigureString() } gender={ gender }
                                               scale={ 0.5 } direction={ 2 }/> }
                    <Base className="avatar-shadow"/>
                    { !hcDisabled && (clubLevel > 0) &&
                        <LayoutCurrencyIcon className="position-absolute top-1 start-1" type="hc"/> }
                    <Flex gap={ 1 } className="button-container flex-column">
                        <Button variant="primary" grow={ true } fullWidth
                                onClick={ event => saveFigureAtWardrobeIndex(index) }>{ LocalizeText('avatareditor.wardrobe.save') }</Button>
                        { figureContainer &&
                            <Button variant="primary" grow={ true } fullWidth onClick={ event => wearFigureAtIndex(index) }
                                    disabled={ (clubLevel > GetClubMemberLevel()) }>{ LocalizeText('generic_usable.button.use') }</Button> }
                    </Flex>
                </LayoutGridItem>
            );
        });
