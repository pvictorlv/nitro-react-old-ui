        ...
        for (let index in items) {
            ...
            if (item.kind === 'file') {
                ...
                img.onload = function(){
                    ...
                    let event = canvas.toDataURL('image/jpeg', 0.65);

                    const userData = roomSession.userDataManager.getUserData(GetSessionDataManager().userId);
                    const bubbleLocation = userData ? GetRoomObjectScreenLocation(roomSession.roomId, userData?.roomIndex, RoomObjectCategory.UNIT) : new NitroPoint();
                    //  const userData = roomObject ? roomSession.userDataManager.getUserData(event.objectId) : new RoomUserData(-1);


                    let chatType = RoomSessionChatEvent.CHAT_TYPE_SPEAK;
                    let styleId = SystemChatStyleEnum.NORMAL;
                    let userType = RoomObjectType.USER;
                    let text = '<a href="' + event + '">Imagem comparptilhada, clique para abrir!</a>';

                    let username = userData.name;
                    const figure = userData.figure;
                    let avatarColor = avatarColorCache.get(figure);

                    const formattedText = RoomChatFormatter(text);
                    const color = (avatarColor && (('#' + (avatarColor.toString(16).padStart(6, '0'))) || null));
                    let imageUrl = getUserImage(figure);
