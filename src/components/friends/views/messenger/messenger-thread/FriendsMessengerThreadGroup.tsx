...
export const FriendsMessengerThreadGroup: FC<{ thread: MessengerThread, group: MessengerThreadChatGroup }> = props => 
{
    ...
    return (
        <Flex fullWidth justifyContent={ isOwnChat ? 'end' : 'start' } gap={ 2 }>
            ...
            <Base
                className={ 'bg-light text-black border-radius mb-2 rounded py-1 px-2 messages-group-' + (isOwnChat ? 'right' : 'left') }>
                ...
                { group.chats.map((chat, index) => <Base key={ index } className="text-break"
                    dangerouslySetInnerHTML={ { __html: chat.message } }
                    onClick={ event => 
                    {
                        if (chat.message.indexOf('a href') > 0) 
                        {
                            var url = chat.message.split('a href="')[1].split('"')[0];
