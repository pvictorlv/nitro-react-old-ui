...
import {AddEventLinkTracker, ChatEntryType, GetRoomEngine, LocalizeText, RemoveLinkEventTracker} from '../../api';
...
export const ChatHistoryView: FC<{}> = props => {
    ...
    return (
        <NitroCardView uniqueKey="chat-history" className="nitro-chat-history" theme="primary-slim">
            <NitroCardHeaderView headerText={LocalizeText('room.chathistory.button.text')}
                                 onCloseClick={event => setIsVisible(false)}/>
            <NitroCardContentView innerRef={elementRef} overflow="hidden" gap={2}>
                <input type="text" className="form-control form-control-sm" placeholder={LocalizeText('generic.search')}
                        ...
                        <Flex alignItems="center" className="p-1" gap={2}>
                            ...
                            {(row.type === ChatEntryType.TYPE_CHAT) &&
                                <div className="bubble-container" style={{position: 'relative'}}>
                                    ...
                                    <div className={`chat-bubble bubble-${row.style} type-${row.chatType}`}
                                         style={{maxWidth: '100%'}}>
