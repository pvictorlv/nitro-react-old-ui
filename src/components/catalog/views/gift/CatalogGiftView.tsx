...
export const CatalogGiftView: FC<{}> = props =>
{
  ...
  useEffect(() =>
  {
    ...
    for (const colorId of giftConfiguration.stuffTypes)
    {
      ...
      if (giftData.colors && giftData.colors.length > 0) newColors.push({
        id: colorId,
        color: ColorUtils.makeColorNumberHex(giftData.colors[0])
      });
    }
    ...
    <NitroCardView uniqueKey="catalog-gift" className="nitro-catalog-gift" theme="primary-slim">
      ...
      <NitroCardContentView className="text-black">
        <FormGroup column>
          ...
          <input type="text" className={ classNames('form-control form-control-sm', receiverNotFound && 'is-invalid') }
                 value={ receiverName } onChange={ (e) => onTextChanged(e) }/>
          { (suggestions.length > 0 && isAutocompleteVisible) &&
            <Column className="autocomplete-gift-container">
              { suggestions.map((friend: MessengerFriend) => (
                <Base key={ friend.id } className="autocomplete-gift-item"
                      onClick={ (e) => selectedReceiverName(friend.name) }>{ friend.name }</Base>
              )) }
            </Column>
          }
          { receiverNotFound &&
            <Base
              className="invalid-feedback">{ LocalizeText('catalog.gift_wrapping.receiver_not_found.title') }</Base> }
        </FormGroup>
        <LayoutGiftTagView figure={ GetSessionDataManager().figure } userName={ GetSessionDataManager().userName }
                           message={ message } editable={ true } onChange={ (value) => setMessage(value) }/>
        <Base className="form-check">
          <input className="form-check-input" type="checkbox" name="showMyFace" checked={ showMyFace }
        ...
        <Flex alignItems="center" gap={ 2 }>
          { selectedColorId &&
            <Base className="gift-preview">
              <LayoutFurniImageView productType={ ProductTypeEnum.FLOOR } productClassId={ colourId }
        ...
        <Column gap={ 1 } className={ isColorable ? '' : 'opacity-50 pointer-events-none' }>
          ...
          <ButtonGroup fullWidth>
            {/*{ colors.map(color => <Button key={ color.id } variant="dark" active={ (color.id === selectedColorId) }*/ }
            {/*                              disabled={ !isColorable } style={ {backgroundColor: color.color} }*/ }
            {/*                              onClick={ () => setSelectedColorId(color.id) }/>) }*/ }
            { colors.map(function (color)
            {
              function getClasses()
              {
                return classNames('btn', 'btn-color', (color.id === selectedColorId) && 'active', !isColorable && 'disabled');
              }
              return <Flex key={ color.id } center className={ getClasses() }
                           onClick={ () => setSelectedColorId(color.id) }>
                <Base className={ 'gift-color' } style={ {backgroundColor: color.color} }></Base>
              </Flex>;
            }) }
          </ButtonGroup>
        </Column>
